{{-- ═══ PAGES CMS ════════════════════════════════════════════════════════════ --}}
<div x-show="$store.adm.page === 'pages-cms'" x-data="pagesCmsPage()">

    <div class="flex items-center justify-between mb-5">
        <div>
            <h2 class="text-base font-bold text-gray-900">Halaman Statis</h2>
            <p class="text-xs text-gray-500 mt-0.5">Kelola konten halaman website</p>
        </div>
    ...
    <div class="adm-card overflow-hidden">
        ...
        <table x-show="!loading" class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr><th class="adm-th">Judul</th><th class="adm-th">Slug</th><th class="adm-th">Status</th><th class="adm-th">Update</th><th class="adm-th"></th></tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                <template x-if="items.length === 0">
                    <tr><td colspan="5" class="td text-center text-gray-400 py-10">Belum ada halaman.</td></tr>
                </template>
                <template x-for="item in items" :key="item.id">
                    <tr class="adm-tr">
                        <td class="td font-medium text-gray-900" x-text="item.title"></td>
                        <td class="td text-xs text-gray-400" x-text="item.slug"></td>
                        <td class="adm-td"><span class="badge" :class="item.is_published ? 'badge-green' : 'badge-gray'" x-text="item.is_published ? 'Aktif' : 'Draft'"></span></td>
                        <td class="td text-xs text-gray-500" x-text="item.updated_at ? new Date(item.updated_at).toLocaleDateString('id-ID',{day:'2-digit',month:'short',year:'numeric'}) : '-'"></td>
                        <td class="adm-td">
        ...
        <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition>
            ...
            <div class="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl" @click.stop>
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 class="font-semibold text-gray-900" x-text="modalMode === 'add' ? 'Tambah Halaman' : 'Edit Halaman'"></h3>
                ...
                <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            ...
                            <input type="text" x-model="form.title" class="adm-input" placeholder="Judul halaman">
                        </div>
                        <div>
                            <label class="adm-label">Slug</label>
                            <input type="text" x-model="form.slug" class="adm-input" placeholder="auto-generate">
                        </div>
                        <div>
                        ...
                        <div class="col-span-2">
                            <label class="adm-label">Konten <span class="text-red-500">*</span></label>
                            <textarea x-model="form.content" class="adm-textarea" rows="10" placeholder="Konten halaman (HTML diperbolehkan)..."></textarea>
                        </div>
                        <div class="col-span-2">
                            <label class="adm-label">Meta Description</label>
                            <textarea x-model="form.meta_description" class="adm-textarea" rows="2" placeholder="Deskripsi untuk SEO..."></textarea>
                        </div>
                    </div>
